...
- **firmware_update.c/.h**: GitHub release checker + pull-based OTA. Periodically resolves the repo's latest release tag, compares it against the running build, and can install the release's update binary over HTTPS (`esp_https_ota`). Exposes status to the web UI and the Home Assistant `update` entity
