...
- Startup no longer blocks on WiFi. The bus bridge, TCP server, register requester and energy tasks now come up whether or not the network does, after a bounded 15-second wait for an address. Previously the wait was indefinite, so a controller that reached provisioning mode never started its TCP bridge at all and stayed a captive portal and nothing else. The web UI also starts up front and listens on every interface, so it is reachable as soon as either the station or the portal has an address
### Fixed
...
- Added web based flash utility for full firmware loading at: https://marklynch.github.io/pool-controller-code/
### Changed
